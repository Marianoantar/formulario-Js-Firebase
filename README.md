...
Luego Agregue un area donde se svan mostrando los usuarios que se van agregando con un boton para eliminarlos de la base de datos.
...
Por ultimo agregue borrar documentos(registros) en firestore. esto se hace sobre la misma parte donde se crea cada boton 'Eliminar' agregandole un addEvenListener con una funcion sin nombre que elimina el documento de la base de datos.
##### Borrar documentos(registros) 
    // crea boton para borrar
            const buttonDelete = document.createElement('button');
            buttonDelete.textContent = 'Eliminar'; // Establece el texto del botón
            buttonDelete.addEventListener('click', () =>{
